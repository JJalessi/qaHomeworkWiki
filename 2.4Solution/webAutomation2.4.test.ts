...
//you can use either reference the const or selector in the test
//my understanding is that it is best practice to use the const so you don't have to type the value that the const is assigned to everytimes since that can change
describe("Employee Manager 1.2", () => {
    ...
    //async means it runs the test in order 1 at a time 
    //it is set up to open employee manager and quit after they all run 
        ...
        */
        ...
        test("A canceled change doesn't persist", async () => {
            ...
            */
            ...
            expect(
                await (await driver.findElement(nameInput)).getAttribute("nameEntry")
            ).toBe("Phillp Weaver");
        });
        ...
        test("A saved change persists", async () => {
            ...
            */
            ...
            await driver.findElement(bernice).click();
            expect(
                await (await driver.findElement(nameInput)).getAttribute("employeeTitle")
            ).toBe("Test Name");
    });
});
...
//don't know what to put in as the getAttribute values